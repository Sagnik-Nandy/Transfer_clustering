...
def load_dataset_tsv(fname, fgenes=None, flabels=None):
    ...
    gene_ids = np.arange(0, data.shape[0]).astype(str)
    ...
    if fgenes is None:
        ...
        gene_ids = np.loadtxt(fgenes, delimiter='\t', dtype=str)
...
def get_transferability_score(W, H, trg_data, reps=100, alpha=0.0, l1=0.75, max_iter=100, rel_err=1e-3):
    ...
    err_best = np.sum(np.abs(trg_data - W_best.dot(H_best))) / float(trg_data.size)  # absolute
    err_curr = np.sum(np.abs(trg_data - W.dot(H))) / float(trg_data.size)  # absolute
...
def get_transferred_data_matrix(W, trg_data, normalize_H2=False, max_iter=100, rel_err=1e-3):
    ...
    while n_iter < max_iter:
        ...
        denom[denom == 0.] = 1e-10  # epsilon safeguard: avoids 0/0 on exact-zero rows instead of aborting
        H *= W.T.dot(trg_data) / denom
        new_err = np.sum(np.abs(trg_data - W.dot(H))) / float(trg_data.size)  # absolute
        # new_err = np.sqrt(np.sum((Xtrg - W.dot(H))*(Xtrg - W.dot(H)))) / float(Xtrg.size)  # frobenius
    ...
    if normalize_H2:
        ...
        n_iter = 0
        err = 1e10
        sparse_rec_err = np.sum(np.abs(trg_data - W.dot(H2))) / float(trg_data.size)  # absolute
        #print n_iter, ': sparse rec error: ', sparse_rec_err
        while n_iter < max_iter:
            n_iter += 1
            denom = W.T.dot(W.dot(H2))
            denom[denom == 0.] = 1e-10  # epsilon safeguard, matches the H-loop above
            H2 *= W.T.dot(trg_data) / denom
...
def get_matching_gene_inds(src_gene_ids, trg_gene_ids):
    ...
    # common_ids = np.array(common_ids, dtype=str)
    common_ids = np.array(common_ids)

    #print('Both datasets have (after processing) {0} (src={1}%,trg={2}%) gene ids in common.'.format(
    #    common_ids.shape[0],
    #    int(float(common_ids.size) / float(src_gene_ids.size)*100.0),
    #    int(float(common_ids.size) / float(trg_gene_ids.size)*100.0)))
    ...
    inds1 = np.zeros(common_ids.shape[0], dtype=int)
    inds2 = np.zeros(common_ids.shape[0], dtype=int)
