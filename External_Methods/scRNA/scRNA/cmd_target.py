...
# 3. CLUSTERING
# --------------------------------------------------
num_cluster = list(map(int, arguments.cluster_range.split(",")))
mixtures = list(map(float, arguments.mixtures.split(",")))
...
for i in range(len(num_cluster)):
    for j in range(len(mixtures)):
        ...
        max_pca_comp = np.ceil(num_cells*0.07).astype(int)
        min_pca_comp = np.max([1, np.floor(num_cells * 0.04).astype(int)])
...
if labels is not None:
    print((' - Class 2 label conversion (class {0:1d}-{1:1d}): '.format(
        int(np.min(labels)), int(np.max(labels))), labels_2_ids))
print('')
...
plt.figure(0)
n = accs.shape[0]
fig, axes = plt.subplots(nrows=4, ncols=int(n / 2))
...
for i in range(n):
    ...
    plt.xticks(np.array(list(range(len(num_cluster))), dtype=float)+0.5, num_cluster, fontsize=8)
    plt.yticks(np.array(list(range(len(mixtures))), dtype=float)+0.5, mixtures, fontsize=8)
    cbar = plt.colorbar()
