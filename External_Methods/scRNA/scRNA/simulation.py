...
def split_source_target(toy_data, true_toy_labels,
    ...
    if mode == 1:
        ...
        true_toy_labels_arr = np.array(true_toy_labels, dtype=int)
        ...
        if cluster_spec == None:
            ...
            ntrg = int(np.floor((len(nclusters) + common) / 2.))
            ...
            # print nclusters
            ntrg = int(np.floor((len(nclusters) + common) / 2.))  # number of clusters in target
        ...
        if len(Cidx) > 0:
            ...
            if shared_trg_size + shared_src_size >= sum(shared_idx):
                ...
                shared_trg_size = int(shared_trg_size - to_take_away)
                shared_src_size = int(shared_src_size - to_take_away)
