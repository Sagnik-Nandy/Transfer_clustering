...
def get_sc3_Ting_labels():
    ...
    for lbl in range(len(SC3_Ting7_results)):
        inds = np.array(SC3_Ting7_results[lbl], dtype=int)-1
...
def gene_filter(data, perc_consensus_genes=0.94, non_zero_threshold=2):
    ...
    lower_bound = float(num_cells)*(1.-perc_consensus_genes)
    upper_bound = float(num_cells)*perc_consensus_genes
...
def transformations(dm, components=5, method='pca'):
    ...
    if method == 'spectral':
        ...
        num_cells = dm.shape[0]
        # J = np.eye(num_cells) - 1./float(num_cells)*np.ones((num_cells, num_cells))
...
def build_consensus_matrix(X):
    ...
    if len(X.shape) == 1:
        X = X[np.newaxis, :]
    n, cells = X.shape
    consensus = np.zeros((cells, cells), dtype=float)
    for i in range(n):
        ...
        t[t == -1] = 0
        # print np.unique(t)
        consensus += np.array(t, dtype=float)
    consensus /= float(n)
    return consensus
