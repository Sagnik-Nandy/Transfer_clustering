...
class NmfClustering(AbstractClustering):
    ...
    def apply(self, k=-1, alpha=1.0, l1=0.75, max_iter=100, rel_err=1e-3):
        ...
        nmf = decomp.NMF(alpha_W=alpha, alpha_H=alpha, init='nndsvdar', l1_ratio=l1, max_iter=max_iter,
    ...
    def print_reconstruction_error(self, X, W, H):
        print(('  Elementwise absolute reconstruction error   : ', np.sum(np.abs(X - W.dot(H))) / float(X.size)))
        print(('  Fro-norm reconstruction error               : ', np.sqrt(np.sum((X - W.dot(H))*(X - W.dot(H)))) / float(X.size)))
...
class NmfClustering_initW(AbstractClustering):
    ...
    def apply(self, k=-1, alpha=1.0, l1=0.75, max_iter=100, rel_err=1e-3):
        ...
        learned_H_t, fixed_W_t_same, n_iter = decomp.non_negative_factorization(X.astype(float), n_components=k, init='custom', random_state=0, update_H=False, H=fixed_W_t.astype(float), alpha=alpha, l1_ratio=l1, max_iter=max_iter, shuffle=True, solver='cd',tol=rel_err, verbose=0)

        init_W = fixed_W_t_same.T
        init_H = learned_H_t.T

        nmf = decomp.NMF(alpha_W=alpha, alpha_H=alpha, init='custom',l1_ratio=l1, max_iter=max_iter, n_components=k, random_state=0, shuffle=True, solver='cd', tol=rel_err, verbose=0)
...
class NmfClustering_fixW(AbstractClustering):
    ...
    def apply(self, k=-1, alpha=1.0, l1=0.75, max_iter=100, rel_err=1e-3):
        ...
        fixed_W = pd.get_dummies(self.labels)
        fixed_W_t = fixed_W.T  # interpret W as H (transpose), you can only fix H, while optimizing W in the code. So we simply switch those matrices (invert their roles).
        learned_H_t, fixed_W_t_same, n_iter = decomp.non_negative_factorization(X_t.astype(float), n_components=k, init='custom', random_state=0, update_H=False, H=fixed_W_t.astype(float), alpha=alpha, l1_ratio=l1, max_iter=max_iter, shuffle=True, solver='cd',tol=rel_err, verbose=0)
        ...
        learned_W, learned_H_fix, n_iter = decomp.non_negative_factorization(X.astype(float), n_components=k, init='custom', random_state=0, update_H=False, H=learned_H_t.T, alpha=alpha, l1_ratio=l1, max_iter=max_iter, shuffle=True, solver='cd',tol=rel_err, verbose=0)
...
class DaNmfClustering(NmfClustering):
    ...
    def get_mixed_data(self, mix=0.0, reject_ratio=0., use_H2=True, calc_transferability=False, max_iter=100, rel_err=1e-3):
        ...
        if reject_ratio > 0.:
            ...
            # inds = np.arange(0, trg_data.shape[1], dtype=int)
            inds = np.argsort(-neg_entropy)  # ascending order
            keep = float(inds.size) * reject_ratio
            inds = inds[:keep]
    ...
    def reject_classifier(self, K, kurts):
        ...
        for i in range(K.shape[1]-2):
            ...
            labels = np.ones(kurts.size, dtype=int)
            labels[sinds[:i+1]] = -1
