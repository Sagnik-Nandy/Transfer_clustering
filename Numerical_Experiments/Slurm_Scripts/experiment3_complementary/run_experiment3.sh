...
#SBATCH -o /path/to/Transfer_clustering/Numerical_Experiments/Slurm_Scripts/experiment3_complementary/Output_Messages/slurm_out_%A_%a_%x_%j_%t.out
#SBATCH -e /path/to/Transfer_clustering/Numerical_Experiments/Slurm_Scripts/experiment3_complementary/Error_Messages/slurm_err_%A_%a_%x_%j_%t.err
...
# --- array math: this experiment is a single fixed scenario (Section 5.2),
# so the array index IS the seed directly -- no regime/grid decoding needed.
seed=$SLURM_ARRAY_TASK_ID

echo "Running seed=$seed"

"$CONDA_PREFIX/bin/python" \
  "$PROJECT_ROOT/Numerical_Experiments/Experiments_Script/run_experiment3.py" \
  "$seed"
