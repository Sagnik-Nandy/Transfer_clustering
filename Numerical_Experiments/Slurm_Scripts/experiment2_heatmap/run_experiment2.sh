...
#SBATCH --mem=32G
#SBATCH --array=0-299                    # 3 regimes x 100 seeds = 300 tasks -- EDIT if REGIMES/N_SEEDS change in run_experiment2.py
...
#SBATCH -o /path/to/Transfer_clustering/Numerical_Experiments/Slurm_Scripts/experiment2_heatmap/Output_Messages/slurm_out_%A_%a_%x_%j_%t.out
#SBATCH -e /path/to/Transfer_clustering/Numerical_Experiments/Slurm_Scripts/experiment2_heatmap/Error_Messages/slurm_err_%A_%a_%x_%j_%t.err
...
# EDIT: keep in sync with REGIME_ORDER / N_SEEDS in run_experiment2.py.
regimes=(R1 R2 R3)
num_seeds=100
...
"$CONDA_PREFIX/bin/python" \
  "$PROJECT_ROOT/Numerical_Experiments/Experiments_Script/run_experiment2.py" \
  "$regime" "$seed"
