...
comparison -- cluster-parallel driver. Mirrors
Final_Lung_Atlas_Analysis/run_lung_atlas_comparison.py's BASE convention
(one (target batch, method) combination per task, all-other-batches-pooled
as the single source spec) -- NOT that file's later multi-source-at-a-time
expansion (all + one-source-at-a-time rows); this dataset has 6 batches, so
that expansion would mean 5 single-source rows per multi-source method
instead of 3 -- add it later as a separate, explicitly-requested step if
wanted, same as it was for the lung atlas.
...
(common.METHOD_LABELS, 8) = 48 tasks. `target_only` uses no sources; every
