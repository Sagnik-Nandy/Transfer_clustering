"""Algorithm 5 (TSClust): Spectral Initialization Followed by Lloyd Refinement.
...
(supplement). Used as the clustering module applied to the
source-projected target observations in Algorithm 3.

Steps (matching Algorithm 5 exactly):
    ...
    3. Initialize via a (1+eps)-approximate K-means solution on the
       spectral coordinates (K-means++, Algorithm 5 line 5).
...
def _spectral_coordinates(X: np.ndarray, K: int) -> np.ndarray:
    """Top-K singular-direction projection: the 'U_hat' of Algorithm 5.
...
def ts_clust(
    ...
    """Algorithm 5: spectral initialization (K-means++ on the top-K
    ...
    K : int
        Number of clusters.
    n_iter : int
        Number of Lloyd refinement iterations (T in Algorithm 5; the
        callers in Algorithm 3 use T = round(2 log n_T)).
    """
    n = X.shape[0]
    rng = np.random.default_rng(random_state)
