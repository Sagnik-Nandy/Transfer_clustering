"""Algorithm 3 (Adaptive Transfer-Assisted Projected Clustering).

From Chakraborty & Nandy, "Transfer Learning in High-Dimensional
Clustering: Minimax Thresholds and Applications in Single-Cell Data",
Section 4: multi-cluster (K > 2) Gaussian mixture models with multiple
source datasets. Builds on:

  - `relaxed_kmeans.RelaxedKMeans`   (Giraud & Verzelen, 2019 relaxed K-means SDP)
  - `ts_clust.ts_clust`              (Algorithm 5, TSClust)

Model (Section 4):
    X_j^(T) = Theta_T Z_j^(T) + noise_j^(T),   Z_j^(T) in {0,1}^K one-hot,
    X_j^(Si) = Theta_Si Z_j^(Si) + noise_j^(Si), for each source i.

Algorithm 3 proceeds:
    ...
    2. For each source i, estimate the source mean matrix Theta_hat_Si
       (label-first cluster means if n_Si is small relative to d, or
       directly via the top-K singular vectors of X_Si otherwise).
    3. Form the source subspace R = col(Theta_hat_S1) + ... + col(Theta_hat_Sm)
       and project the target observations onto it.
    4. Cluster the projected target observations with TSClust (Algorithm 5)
       to get the source-based estimator.
    5. Adaptively choose between the target- and source-based estimators
       using a validation statistic and threshold, analogous to Algorithm 2's
       two-community construction.
"""
...
# ---------------------------------------------------------------------------
# Source mean / subspace estimation
# ---------------------------------------------------------------------------
...
def estimate_source_means(
    ...
    """Estimate the source mean matrix Theta_hat_S (d, K), per Section 4.
    ...
    - n_S >= d ("4 <= d <~ n_Si" regime): estimate the source signal
      subspace directly as the top-K right singular vectors of X_S.
    """
    n_S, d = X_S.shape
    if n_S < d:
        ...
        return cluster_means(X_S, Z_hat)
    _, _, Vt = np.linalg.svd(X_S, full_matrices=False)
    return Vt[:K].T                                     # (d, K)


def source_subspace_projector(
    theta_hats: Sequence[np.ndarray], tol: float = 1e-8, max_rank: Optional[int] = None,
    normalize: bool = False,
) -> np.ndarray:
    """Orthonormal basis U_hat_S of R = col(Theta_hat_S1) + ... + col(Theta_hat_Sm),
    the source subspace of Section 4.
    ...
    orthonormal columns unpivoted QR would produce. If `max_rank` is
    given, the basis is additionally truncated to its `max_rank` leading
    singular directions (by singular value). By default no per-dataset
    normalization is applied first, so whichever pooled mean matrix has
    the largest magnitude dominates those leading directions. If
    `normalize` is True, each individual cluster-mean column (i.e. every
    column of every Theta_hat, not each Theta_hat as a whole) is rescaled
    to unit L2 norm before the SVD, so every cluster-mean direction from
    every dataset competes on equal footing regardless of its raw
    magnitude.
    """
    theta_hats = list(theta_hats)
    if normalize:
        theta_hats = [th / np.linalg.norm(th, axis=0, keepdims=True) for th in theta_hats]
    M = np.column_stack(theta_hats)
...
# ---------------------------------------------------------------------------
# Validation statistic and threshold (Section 4)
# ---------------------------------------------------------------------------


def validation_statistic_mult(Z: np.ndarray, X_T: np.ndarray, sigma_T2: float) -> float:
    """Multi-cluster validation statistic S_hat(Z): a normalized sum of
    squared pairwise gaps between estimated cluster means, de-biased by
    the noise level."""
    n_T, K = Z.shape
    d = X_T.shape[1]
    n_a = Z.sum(axis=0)
...
def validation_threshold_mult(sigma_T2: float, d: int, K: int, n_T: int, D0: float = 1.0) -> float:
    """Multi-cluster validation threshold t_n: t_n = D0 K sigma_T^2 (sqrt(dK/n_T) + 1)."""
...
def estimate_noise_variance_rankK(X_T: np.ndarray, K: int) -> float:
    ...
    X_T -- the K>2 analogue of `two_community.estimate_noise_variance_rank1`."""
    n_T, d = X_T.shape
...
def calibrate_D0_bootstrap_mult(
    ...
    + 1)) -- the K>2 analogue of `two_community.calibrate_C0_bootstrap`.
    ...
    *boundary* rather than at zero signal (a zero-signal null answers "is
    ...
    boundary separation is the natural generalization of the K=2 recovery
    threshold max(1, (d/n_T)^(1/4)) whose square matches
...
def _target_branch(X_T: np.ndarray, K: int, relaxed_kmeans_kwargs: Dict) -> np.ndarray:
    """Algorithm 3, line 1: target-based estimator via relaxed K-means, as a one-hot Z."""
...
def _source_branch(
    ...
    """Algorithm 3, source-based estimator via subspace projection + TSClust.
...
def target_source_pooled_subspace_estimate(
    ...
    called with `restrict_basis_rank=True`, and the K>2 analogue of
    ...
    reserved for a separate target-only branch.
        ...
        5. cluster X_hat_T with TSClust (Algorithm 5), exactly as in
           `_source_branch`.
    ...
    With `m` sources, R's unrestricted rank can reach `(m+1)*K` -- more
    dimensions than TSClust's downstream K-cluster search actually uses,
    so weakly-estimated mean matrices can contribute directions that are
    largely noise. If `restrict_basis_rank` is True, R is capped at its K
    leading singular directions (matching TSClust's K), computed from the
    stacked mean matrices. By default no per-dataset normalization is
    applied, so whichever dataset's Theta_hat has the largest magnitude
    dominates those leading directions; if `normalize` is also True, every
    individual cluster-mean column of every Theta_hat (target's and each
    source's) is rescaled to unit norm before this truncation, so every
    cluster-mean direction competes on equal footing regardless of its raw
    magnitude (`normalize` has no effect when `restrict_basis_rank` is
    False, since per-column scaling doesn't change the span an untruncated
    SVD basis spans). Default False keeps the full-span behavior.
    """
    ...
    Q_R = source_subspace_projector(theta_hats, max_rank=max_rank, normalize=normalize)
...
def pooled_subspace_estimate(
    ...
    """Raw-data pooling baseline, K>2 analogue of
    `two_community.pooled_subspace_estimate` (not the paper's Algorithm
    4 -- see `target_source_pooled_subspace_estimate` for that): an
    alternative to `AdaptiveProjectedClustering`'s hard target/source
...
class AdaptiveProjectedClustering:
    ...
    selection : {"formula", "bootstrap", "manual"}
        - "formula": the literal Section 4 threshold with a
    ...
    D0 : float
        Constant in the multi-cluster validation threshold t_n; used when
        `selection == "formula"`.
    ...
    def fit_predict(
        ...
        # Final adaptive choice, per Algorithm 3.
