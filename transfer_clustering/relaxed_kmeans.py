...
`RelaxedKMeans.solver`:
  ...
  - "ts_clust": skips the SDP entirely and clusters X directly with
    `ts_clust` (Algorithm 5, TSClust -- spectral initialization on X's own
    top-K singular directions, then Lloyd refinement), exactly the routine
    ...
    fallback rather than a faithful reimplementation of Algorithm 3's
    target branch.
"""
