...
pipeline of Section 4 / Algorithm 3 (Giraud & Verzelen's relaxed
    ...
    K : int
        Number of clusters.
          - K == 2: uses the two-community machinery of Section 2
            ...
            Section 2.3 adaptive selection, Algorithm 2).
          - K > 2: uses the multi-cluster machinery of Section 4
            (Algorithm 3: Giraud & Verzelen's relaxed K-means SDP +
            TSClust / Algorithm 5).
    oracle : {"target", "source", None}
        If given (K == 2 only), bypasses the adaptive selection and runs
        Algorithm 1 directly under the stated oracle assumption.
        If None (default), uses the oracle-free adaptive selection
        (Algorithm 2 for K=2, Algorithm 3 for K>2).
    **kwargs
