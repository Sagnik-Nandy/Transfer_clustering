...
#SBATCH --mem=16G
#SBATCH --array=0-75                     # 4 batches x 19 (target_only:1 + 4 multi-source methods x
                                          # 4 source-specs each [all + 3 single-source] + 2 comparators:1
                                          # [tlgmm, gdec_gcnfree -- scrna dropped, see common.py])
                                          # = 76 tasks -- EDIT if common.BATCHES, common.METHOD_LABELS,
...
#SBATCH -o /home/nandy.15/Research/Transfer_clustering/Final_Lung_Atlas_Analysis_PCA100/Output_Messages/slurm_out_%A_%a_%x_%j_%t.out
#SBATCH -e /home/nandy.15/Research/Transfer_clustering/Final_Lung_Atlas_Analysis_PCA100/Error_Messages/slurm_err_%A_%a_%x_%j_%t.err
...
# loading, PCA projection, and metrics), plus torch (method_gdec.py's
# SDAE/DEC pipeline, CPU-only is fine at this data scale) -- see
# run_lung_atlas_comparison.ipynb's header note. scrna/cvxopt are NOT
# needed here (NMF dropped -- see common.py's module docstring). UNLIKE
...
# Verified 2026-07-10 (base Final_Lung_Atlas_Analysis run): all_amp_projects
# has scanpy/torch/cvxopt/cvxpy all importable (torch needs the
# LD_LIBRARY_PATH export below; scanpy needed a typing_extensions>=4.16
# upgrade -- anndata's pin -- done in that env).
...
cd "$PROJECT_ROOT/Final_Lung_Atlas_Analysis_PCA100"
...
"$CONDA_PREFIX/bin/python" \
  "$PROJECT_ROOT/Final_Lung_Atlas_Analysis_PCA100/run_lung_atlas_comparison.py" \
  "$task_id"
