...
runs all (target batch, method, source) combinations sequentially in one
process, which is fine for a single sanity-check batch but not for the full
4-batch comparison (adaptive_multi_source alone re-solves the relaxed
K-means SDP ~34 times per batch -- see method_ours.py). This script runs
exactly ONE (target batch, method, source) combination per invocation, so
each can be its own Slurm array task (Slurm_Scripts convention: matches
run_experiment3.py / run_experiment4.py's "one seed per array task", just
indexed over (batch, method, source) here instead of seed).

The `source` dimension: `target_only` uses no sources at all, and the
comparators (tlgmm/scrna/gdec_gcnfree) always use every other batch pooled
together ("all"), same as before. But each of the 4 "ours" methods that
take a `sources` list (multi_source_pooled, pooled_concat,
target_source_pooled, adaptive_multi_source) is now run BOTH with all 3
other batches pooled ("all", the original behavior) AND, separately, with
each individual other batch as the sole source ("Dropseq_2" etc.) -- so a
method's per-source sensitivity can be inspected directly, not just its
pooled-source performance. This doesn't change any of those methods'
implementations -- they already accept an arbitrary `sources` list, a
single-source run is just `sources=[X_one_batch]` instead of all 3.
...
def task_grid():
    ...
    over source_spec:
      ...
      - the 3 comparators: a single row, source_spec="all".
    """
    grid = []
...
def run_one(task_id: int) -> dict:
    ...
    if source_spec is None:
        ...
        sources = [batch_data[source_spec][0]]
